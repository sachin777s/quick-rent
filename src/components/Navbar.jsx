      ...
      <nav className="py-2 px-2 sm:px-4 mx-auto max-w-[1680px] h-[65px] shadow-md flex items-center justify-between">
        ...
        <div className="sm:hidden flex items-center">
          <button
            ...
            onClick={toggleMenu}
          >
            {isMenuOpen ? <IoMdClose size={32} /> : <RxHamburgerMenu size={32} />}
          </button>
        </div>

        {/* Desktop Buttons */}
        <div className="hidden sm:flex items-center gap-2">
          <button className="px-8 py-2 rounded-full border-2 border-[var(--main-color)] text-[var(--main-color)] transition-all duration-300 hover:text-white hover:bg-[var(--main-color)] hover:shadow-[0_0_11px_-3px_var(--main-color)] active:scale-95">
            Login
          </button>
          <button className="px-8 py-2 rounded-full border-2 border-[var(--main-color)] text-white bg-[var(--main-color)] transition-all duration-300 hover:bg-white hover:text-[var(--main-color)] hover:shadow-[0_0_11px_-3px_var(--main-color)] active:scale-95">
            Signup
          </button>
        </div>
      </nav>
